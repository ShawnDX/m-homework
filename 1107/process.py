...
def log_transaction(transaction_type, date, stock, number_of_shares, price,
    ...
    '''
    file = open(ledger_file, 'a', encoding='utf-8')
    file.write('{},{},{},{},{},{}\n'.format(transaction_type, date, stock,
                                            number_of_shares, price, fees))
    file.close()
...
def buy(date, stock, available_capital, stock_prices, fees, portfolio,
    ...
    Input:
        ...
        available_capital (float): the total (maximum) amount to spend, this must also cover fees
    ...
    price = round(stock_prices[date + 1][stock], 2)
    num = int((available_capital - fees) / price)
    cost = round(-num * price - fees, 2)
    portfolio[stock] = num

    log_transaction(transaction_type, date, stock, num, price, cost)
...
def sell(date, stock, stock_prices, fees, portfolio, ledger_file):
    ...
    price = round(stock_prices[date + 1][stock], 2)
    num = portfolio[stock]
    earn = round(price * num - fees, 2)

    log_transaction(transaction_type, date, stock, num, price, earn)
...
def create_portfolio(available_amounts, stock_prices, fees):
    ...
    Example:
        Spend 1000 for each stock (including 40 fees for eacFFh purchase):
        >>> N = sim_data.shape[1]
    ...
    for i in range(len(available_amounts)):
        available_capital = available_amounts[i]
        buy(0, i, available_capital, stock_prices, fees, portfolio,
            ledger_file)
